...
For files longer than ~25 seconds, use `audd_recognize_enterprise`,
...
```c
audd_client_t *client = audd_client_new("aud_xxx_your_token", NULL);
```
...
unparsed JSON. Use it to read undocumented or beta fields not yet exposed
as typed properties:
...
audd_client_t *client = audd_client_new("aud_xxx", &opts);
```
...
```c
/* Computed locally from token + radio_id; no round-trip. */
char category[10];
audd_streams_derive_longpoll_category(client, /*radio_id=*/1, category);
...
audd_longpoll_run(client, category, NULL, &cb, &handle);
...
> `audd_recognize_enterprise` for files longer than 25 seconds) instead.
