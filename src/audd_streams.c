                                             ...
                                             const audd_streams_set_callback_url_options_t *options)
{
    ...
    /* extras first; typed params (url) win on collision via later writes */
    size_t extras_pairs = 0;
    const char **extras = options ? options->extra_parameters : NULL;
    if (extras) {
        for (size_t i = 0; extras[i] != NULL && extras[i + 1] != NULL; i += 2) extras_pairs++;
    }
    const size_t total = extras_pairs * 2 + 3; /* + "url", final_url, NULL */
    const char **fields = (const char **)audd_malloc(total * sizeof(*fields));
    if (fields == NULL) { audd_free(final_url); return AUDD_ERR_OUT_OF_MEMORY; }
    size_t f = 0;
    if (extras) {
        for (size_t i = 0; extras[i] != NULL && extras[i + 1] != NULL; i += 2) {
            fields[f++] = extras[i];
            fields[f++] = extras[i + 1];
        }
    }
...
audd_error_t audd_streams_add(audd_client_t *client, const audd_add_stream_request_t *req)
{
    ...
    size_t extras_pairs = 0;
    if (req->extra_parameters) {
        for (size_t i = 0; req->extra_parameters[i] != NULL && req->extra_parameters[i + 1] != NULL; i += 2) extras_pairs++;
    }
    const size_t total = extras_pairs * 2 + 7; /* url+rid+callbacks (6) + NULL */
    const char **fields = (const char **)audd_malloc(total * sizeof(*fields));
    if (fields == NULL) return AUDD_ERR_OUT_OF_MEMORY;
    size_t f = 0;
    if (req->extra_parameters) {
        for (size_t i = 0; req->extra_parameters[i] != NULL && req->extra_parameters[i + 1] != NULL; i += 2) {
            fields[f++] = req->extra_parameters[i];
            fields[f++] = req->extra_parameters[i + 1];
        }
    }
    ...
    fields[f] = NULL;
    audd_error_t e = streams_post(client, "/addStream/", fields, AUDD_RETRY_MUTATING, NULL);
    audd_free((void *)fields);
    return e;
}
