...
typedef struct {
    ...
    /** Per-request timeout for the enterprise endpoint. Default: 3600s (1h). */
...
typedef struct {
     ...
     *  NULL-terminated alternating key/value array: {"k1","v1","k2","v2",NULL}.
     *  Typed params win on collision. NULL to send no extras. */
...
typedef struct {
     ...
     *  collision. NULL to send no extras. */
    const char **extra_parameters;
...
typedef struct {
    ...
    const char **return_metadata;
    /** Additional form fields the typed params don't cover, as a
     *  NULL-terminated alternating key/value array. Typed params (`url`) win
     *  on collision. NULL to send no extras. */
    const char **extra_parameters;
...
typedef struct {
    ...
    /** Additional form fields the typed params don't cover, as a
     *  NULL-terminated alternating key/value array. Typed params (`url`,
     *  `radio_id`, `callbacks`) win on collision. NULL to send no extras. */
    const char **extra_parameters;
...
/**
 * One-step variant of audd_longpoll_run keyed on radio_id. Derives the
 * 9-char longpoll category internally from the client's stored api_token
 * + radio_id, then runs the longpoll. The category is not exposed.
 *
 * Use this when you have both the api_token and the radio_id (the common
 * case). Use audd_longpoll_run with a pre-derived category string when
 * the token must stay server-side and only the category travels to a
 * tokenless consumer (browser/widget/embedded).
 *
 * Returns AUDD_ERR_AUTHENTICATION if the client was constructed without
 * an api_token. All other semantics — preflight callback URL check,
 * blocking behavior, callback dispatch, close handle — match
 * audd_longpoll_run exactly.
 */
AUDD_API audd_error_t audd_longpoll_run_by_radio_id(audd_client_t *client,
                                                     int radio_id,
                                                     const audd_longpoll_options_t *options,
                                                     const audd_longpoll_callbacks_t *callbacks,
                                                     audd_longpoll_t **out_handle);
